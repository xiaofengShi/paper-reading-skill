...
**先分清三种顺序。** 原论文按「模型与预训练 → RL 方法及分析 → MOPD2 → 最终模型评测 → 支撑训练的基础设施」展开；本页按理解依赖组织为「全局 → 一轮 RL → GRS/GAR → 运行系统 → MOPD2 → 实验证据 → 综合」。这样先知道学习信号如何产生，再看质量评分和系统怎样使大规模训练可行，最后对照实验。下方五步图则表示**模型训练与结果检验的阶段关系**：前四步是训练，第五步是评测，不是一次额外的训练更新。运行系统贯穿 RL，不位于 RL 与 MOPD2 之间的时间节点。本页章节顺序因此既不照搬论文目录，也不等同于五步图的时间顺序。（PDF pp. 3, 7–9, 24–32）

| 读图时看到的顺序 | 它回答的问题 | 本页对应位置 |
|---|---|---|
| 模型阶段：基础模型 → Agent 中训/SFT → 混合 RL → MOPD2 → 评测 | 模型先后经历什么；最后一步只检验结果 | 下方五步图 |
| 论文叙述：模型/预训练 → RL/分析 → MOPD2 → 最终评测 → 基础设施 | 作者按什么章节展开材料 | 原论文正文 |
| 本页阅读：全局 → RL 循环 → 评分 → 运行系统 → 蒸馏 → 实验 → 综合 | 哪个概念要先懂，才能读懂下一节 | 左侧七章目录 |

**训练阶段**从多模态预训练出发，经过 Agent 中期训练、短 SFT、混合任务 RL，再用 MOPD2 整合不同领域教师的能力。沿下面的阶段图，重点看每一步解决什么缺口、把什么交给下一步；末尾评测检查训练后的结果。图表示阶段关系，不暗示每个阶段只运行一次。（PDF pp. 3, 7–9, 24–26）

```paper-path
{
  "title":"四段训练与一次评测：从基础模型到最终结果",
  "intro":"前四步是模型训练，第五步是结果评测；运行系统贯穿 RL，不是两次训练之间的独立阶段。",
  "stages":[
    ...
    {"title":"最终模型评测","role":"结果检验，不是训练","action":"Pro 与 Flash 在代码、通用、视觉、安全等任务上评测，并与前代及其他模型比较。","why":"跨任务结果检验整条训练链是否产生有用能力，但不能单独隔离某个模块的贡献。","output":"论文的系统级性能结论及其适用范围。","source":"PDF pp. 25–26, Table 3"}
  ]
}
```
