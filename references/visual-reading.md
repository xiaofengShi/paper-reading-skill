...
Archify's typed JSON and self-contained HTML are useful for compact, explorable workflows, component relationships, data flow, sequences, and lifecycles. Choose its diagram type by the actual paper content. Keep one central path per Archify artifact and put long explanations in its detail cards or the reading document. Use authored links/views for paper locations when supported; otherwise cite the sources in the adjacent Markdown. Retain the source JSON and validate the delivered HTML using Archify's documented commands. Its renderer's validations check diagram structure and layout, **not** whether scientific claims or citations are true; cross-check those against the paper separately.

For concept maps, proof graphs, experiment matrices, and dense numerical comparisons, use Mermaid/SVG/HTML tables or another suitable renderer when they communicate better. Do not make Archify a required dependency. If interactive HTML is delivered, ensure its core information is also available in static text without JavaScript or a network connection. Exported images alone are insufficient for a deep read.
