...
When a document has both a chronology and an explanatory reading route, label them separately before drawing either one. Say whether each arrow means time, dependency, data movement, or feedback. Put cross-cutting infrastructure beside the phase it supports rather than implying that it is a later training phase. Label every quantitative redraw as the document author's redraw, identify the original paper figure and panel, and keep an original excerpt nearby when the original curve or layout carries information the redraw omits.
