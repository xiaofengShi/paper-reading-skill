...
- Give the first screen paper-specific orientation rather than decorative shapes or generic slogans. For before/after results, show both values on a shared scale, label the metric and unit, and keep the numerical table. Keep inline formulas in their sentences and display formulas in separate blocks. Inspect the final HTML for broken Markdown markers, isolated inline formulas, overflow, and figures whose caption only points elsewhere.
